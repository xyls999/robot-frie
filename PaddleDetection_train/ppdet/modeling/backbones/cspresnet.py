...
# FireDetect 当前 PP-YOLOE+ CRN-S 使用这个文件里的 CSPResNet 做 backbone。
# 主要调参入口在 YAML：
# - layers / depth_mult：控制网络深度，影响特征表达和速度。
# - channels / width_mult：控制通道宽度，影响精度、显存和 FPS。
# - return_idx: [1, 2, 3]：返回 stride 8/16/32 三层特征给 neck。


class ConvBNLayer(nn.Layer):
    """Conv + BatchNorm + activation，是 CSPResNet/CustomCSPPAN 的基础卷积单元。"""
...
class RepVggBlock(nn.Layer):
    """训练时 3x3+1x1 双分支，部署时可融合成单个 3x3 卷积，提高推理友好性。"""
...
class BasicBlock(nn.Layer):
    """CSPResNet 的基础残差块；shortcut 保留原特征，有利于稳定训练。"""
...
class CSPResStage(nn.Layer):
    """一个 CSP stage：下采样后分流，一路轻变换，一路堆叠 BasicBlock，再 concat 融合。"""
