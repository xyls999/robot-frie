...
# FireDetect 当前 PP-YOLOE+ 使用 CustomCSPPAN 做 neck。
# 它负责把 backbone 的 stride 8/16/32 特征互相融合：
# - FPN 路径：高层语义上采样，补给低层细节。
# - PAN 路径：低层定位信息下采样，回流到高层。
# 小目标漏检时，neck 的输入尺寸、stride 8 特征质量、nms/threshold 往往比盲目加深模型更关键。
...
class SPP(nn.Layer):
    """Spatial Pyramid Pooling：用多个池化核扩大最高层感受野，增强上下文。"""
...
class CSPStage(nn.Layer):
    """neck 内部的 CSP 融合块；block_num/stage_num 越大表达越强，但更慢。"""
...
class CustomCSPPAN(nn.Layer):
    """PP-YOLOE+ 的多尺度特征融合网络，输出给 PPYOLOEHead 做检测。"""
    ...
    def __init__(self,
        ...
        for i in reversed(range(self.num_blocks - 1)):
            # PAN stage：再从浅层向深层下采样融合，增强定位和语义一致性。
            pan_routes.append(
                ConvBNLayer(
