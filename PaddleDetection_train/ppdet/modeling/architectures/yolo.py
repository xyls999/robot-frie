...
#
# FireDetect 当前配置虽然写的是 architecture: YOLOv3，但实际算法不是老 YOLOv3：
# - backbone = CSPResNet
# - neck = CustomCSPPAN
# - yolo_head = PPYOLOEHead
# 这个类只是 PaddleDetection 里通用的 YOLO 容器，负责把 backbone -> neck -> head 串起来。
...
class YOLOv3(BaseArch):
    ...
    def from_config(cls, cfg, *args, **kwargs):
        # backbone
        # 根据 YAML 中 YOLOv3.backbone 创建主干，例如 CSPResNet。
        backbone = create(cfg['backbone'])

        # fpn
        # neck 接收 backbone.out_shape，决定多尺度特征融合层的输入通道。
        kwargs = {'input_shape': backbone.out_shape}
        neck = create(cfg['neck'], **kwargs)

        # head
        # head 接收 neck.out_shape，决定每个检测尺度的分类/回归分支输入通道。
    ...
    def _forward(self):
        # 1. CSPResNet 提取 stride 8/16/32 三层图像特征。
        body_feats = self.backbone(self.inputs)
        if self.for_mot:
            neck_feats = self.neck(body_feats, self.for_mot)
        else:
            # 2. CustomCSPPAN 做自顶向下 + 自底向上的多尺度融合。
        ...
        if self.training:
            # 3a. 训练阶段：PPYOLOEHead 直接返回分类、IoU、DFL 等 loss。
            ...
            # 3b. 推理/导出阶段：head 输出候选框分布，再在 post_process 中解码和 NMS。
            ...
            if self.for_mot:
                ...
                if self.return_idx:
                    ...
                    # FireDetect 当前走这里：PPYOLOEHead.post_process 完成 bbox 解码和 MultiClassNMS。
