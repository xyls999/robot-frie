...
# FireDetect 当前使用 PPYOLOEHead 作为检测头。
# 它是 anchor-free 设计：每个特征点直接预测类别分数和到框边界的距离分布。
# 主要可调点在 YAML 的 PPYOLOEHead：
# - static_assigner_epoch：ATSS -> TaskAligned 的切换轮次。
# - loss_weight：分类、IoU、DFL 三类 loss 权重。
# - assigner.topk / nms.*：影响召回、误检和重复框。


class ESEAttn(nn.Layer):
    """Effective SE 注意力块：用全局平均特征给通道加权，增强有效特征。"""
...
class PPYOLOEHead(nn.Layer):
    """PP-YOLOE/PP-YOLOE+ anchor-free 检测头，负责分类、框回归、loss 和后处理。"""
    ...
    def __init__(self,
        ...
        # 训练前期用 static_assigner(ATSS) 更稳定，后期用 TaskAligned 让正样本更贴合模型能力。
        ...
        # pred head
        # pred_cls 输出 num_classes 个类别分数；pred_reg 输出 4 个方向的离散距离分布。
        ...
        # projection conv
        # DFL 投影层：把离散距离分布转成连续边框距离，通常不参与学习。
    ...
    def forward_train(self, feats, targets, aux_pred=None):
        # 根据三层特征图生成 anchor points。PP-YOLOE 虽然叫 anchor-free，
        # 仍需要网格点作为预测框中心/参考点。
        ...
        for i, feat in enumerate(feats):
            # 每个尺度分别预测分类和回归；stride 8 主要负责小目标，stride 32 负责大目标。
    ...
    def _generate_anchors(self, feats=None, dtype='float32'):
        ...
        # 推理阶段根据 eval_size 或实际特征图大小生成固定网格点。
        anchor_points = []
        stride_tensor = []
