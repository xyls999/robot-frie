...
class Trainer(object):
    ...
    def _model_to_static(self, model, input_spec, prune_input=True):
        if prune_input:
            # Paddle 3.x defaults to the new PIR path. Some PaddleDetection
            # release/2.9 export models still need the old IR during dy2static.
            paddle_version = version.parse(paddle.__version__)
            use_old_ir = (
                paddle_version >= version.parse('3.0.0b2') or
                paddle_version == version.parse('0.0.0')
            ) and not self.cfg.get("export_with_pir", False)
            if use_old_ir:
