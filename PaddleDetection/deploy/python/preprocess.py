...
"""
PaddleDetection 推理前处理算子。

本赛题当前 infer_cfg.yml 使用的典型链路是：
1. Resize：把原图缩放到模型输入尺寸。
2. NormalizeImage：按训练时的 mean/std 做归一化。
3. Permute：把 HWC 图片转成 Paddle 模型常用的 CHW。

其他算子来自官方模板，主要用于不同模型结构或关键点/姿态等任务。
"""
...
def decode_image(im_file, im_info):
    ...
    if isinstance(im_file, str):
        # 用 imdecode 而不是 cv2.imread，能更稳地处理带中文或特殊字符的路径。
        ...
        # 也支持外部直接传入 np.ndarray，方便批量或视频帧复用。
        im = im_file
    # im_shape 是当前图像高宽，scale_factor 初始为 1，后续 Resize 会更新它。
...
class Resize_Mult32(object):
    ...
    def generate_scale(self, img):
        ...
        # 根据 limit_type 决定限制长边、短边，或按长边直接缩放。
...
class Resize(object):
    ...
    def __call__(self, im, im_info):
        ...
        im_channel = im.shape[2]
        # generate_scale 返回 y/x 两个方向的缩放比例；keep_ratio=False 时会强行拉伸到目标尺寸。
        im_scale_y, im_scale_x = self.generate_scale(im)
        im = cv2.resize(
            im,
            None,
            ...
            interpolation=self.interp)
        im_info['im_shape'] = np.array(im.shape[:2]).astype('float32')
        # scale_factor 会传给模型，用于把检测框坐标还原到原图尺度。
        im_info['scale_factor'] = np.array(
            [im_scale_y, im_scale_x]).astype('float32')
        return im, im_info
    ...
    def generate_scale(self, im):
        ...
        if self.keep_ratio:
            ...
            # 非等比例缩放：高、宽分别缩放到指定尺寸，速度简单但可能改变目标形状。
...
class NormalizeImage(object):
    ...
    def __call__(self, im, im_info):
        ...
        if self.is_scale:
            # 常见训练配置会先把 0-255 像素缩放到 0-1。
            scale = 1.0 / 255.0
            im *= scale

        if self.norm_type == 'mean_std':
            # 按通道执行 (image - mean) / std，必须与训练配置保持一致。
...
class Permute(object):
    ...
    def __call__(self, im, im_info):
        ...
        Returns:
            ...
            im_info (dict): info of processed image
        """
        # OpenCV/PIL 读出的图片通常是 HWC；Paddle 检测模型输入通常是 CHW。
...
class PadStride(object):
    ...
    def __call__(self, im, im_info):
        ...
        # pad 到 stride 的整数倍，避免 FPN 等结构下采样时尺寸不能整除。
...
class LetterBoxResize(object):
    ...
    def letterbox(self, img, height, width, color=(127.5, 127.5, 127.5)):
        # letterbox：等比例缩放后补边，常见于 YOLO 类模型，能避免直接拉伸图像。
...
class WarpAffine(object):
    ...
    def __call__(self, im, im_info):
        ...
        if self.keep_res:
            # keep_res 模式尽量保留原图分辨率，只调整到 pad 对齐后的尺寸。
            ...
            # 固定输入尺寸模式，通常用于 CenterNet/关键点类模型。
            s = max(h, w) * 1.0
...
def preprocess(im, preprocess_ops):
    ...
    for operator in preprocess_ops:
        # operator 是 Resize/NormalizeImage/Permute 等可调用对象。
